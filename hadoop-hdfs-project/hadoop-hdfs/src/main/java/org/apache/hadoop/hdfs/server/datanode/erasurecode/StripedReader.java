...
class StripedReader {
  ...
  private int bufferSize;//最好定义成cellsize的大小
  private int[] successList; //记录的是能够读取的DataNode在条带中的偏移量们

  private final int minRequiredSources;  // 一个条带中cell的数量和ec策略中系统节点数量二中的最小值
  ...
  private final List<StripedBlockReader> readers; //readers中都是一个个reader，一个reader对应一个可读的datanode
  ...
  void init(String codecName) throws IOException {
    initReaders(codecName);
  ...
  private void initReaders(String codecName) throws IOException {
  ...
  ByteBuffer[] getInputBuffers(int toReconstructLen) {
    ...
    if (successList.length < dataBlkNum) {   //补上缺失的位置
      for (int i = 0; i < zeroStripeBuffers.length; i++) {
   ...
   * @throws IOException
   */
  void readMinimumSources(int reconstructLength,String codecName) throws IOException {
    CorruptedBlocks corruptedBlocks = new CorruptedBlocks();
    try {
      successList = doReadMinimumSources(reconstructLength, corruptedBlocks,codecName);
    } finally {
  ...
  int[] doReadMinimumSources(int reconstructLength,
                             CorruptedBlocks corruptedBlocks, String codecName)
      throws IOException {
    ...
    if ("piggyBack".equals(codecName)) {
      ...
      for (int i = 0; i < minRequiredSources; i++) {
        ...
        // 只需要根据successList[i]的值，便可以决定下面的buffer只读取多少
        int toRead = getReadLength(liveIndices[successList[i]],
                reconstructLength);
        if (toRead > 0) {
          Callable<Void> readCallable =
                  reader.readFromBlock(toRead, corruptedBlocks);  //每次对各个节点上的每个块读取toRead长度的字节
