...
public class DFSStripedInputStream extends DFSInputStream {
  ...
  synchronized void blockSeekTo(long target) throws IOException {
    ...
    //pos:读文件的游标，blockEnd：游标所在的块的最后一个字节在整个文件中的游标，currentLocatedBlock:
   // 游标对应的块的在哪个块组中，块组ID和块的ID就是相当于IP中的网络号和主机号的区别
  }
  ...
  private void readOneStripe(CorruptedBlocks corruptedBlocks)
    ...
    final int stripeIndex = (int) (offsetInBlockGroup / stripeLen); //当前游标属于第几个stripe
    final int stripeBufOffset = (int) (offsetInBlockGroup % stripeLen);
    final int stripeLimit = (int) Math.min(currentLocatedBlock.getBlockSize() //该处curent。。指的一个块组
        - (stripeIndex * stripeLen), stripeLen);// 下一个条带数据，因为可能是最后一个不满的条带
    StripeRange stripeRange =
        new StripeRange(offsetInBlockGroup, stripeLimit - stripeBufOffset);//块组偏移量到条带尾巴的范围
  ...
  protected synchronized int readWithStrategy(ReaderStrategy strategy)
    ...
    if (pos < getFileLength()) {
      try {
        ...
        // 一次性把该块剩下的所有内容读了，或者按照配置中的每次读取长度来
        int realLen = (int) Math.min(len, (blockEnd - pos + 1L));
        //读最后一个快，因为最后一个块可能不满128M
