...
public class StripedBlockUtil {
  ...
  public static StripingChunkReadResult getNextCompletedStripedRead(
    ...
    try {
      if (timeoutMillis > 0) {
        future = readService.poll(timeoutMillis, TimeUnit.MILLISECONDS);  //service中的就是之前提交的读取Stripe的任务
      } else {
   ...
   */
  // 计算出一个Stripe信息：1.每个cell（包括parity cell）在internal block中的范围
  ...
  public static AlignedStripe[] divideOneStripe(ErasureCodingPolicy ecPolicy,
    ...
    StripingCell[] cells = getStripingCellsOfByteRange(ecPolicy, cellSize,
        blockGroup, rangeStartInBlockGroup, rangeEndInBlockGroup); //将一个条带的数据切割成cell大小的数组
