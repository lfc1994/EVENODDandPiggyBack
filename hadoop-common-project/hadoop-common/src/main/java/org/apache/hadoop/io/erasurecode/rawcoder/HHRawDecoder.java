...
public class HHRawDecoder extends RawErasureDecoder {
    ...
    public HHRawDecoder(ErasureCoderOptions coderOptions) {
        super(coderOptions);
    ...
    protected void doDecode(ByteBufferDecodingState decodingState) throws IOException {
        ...
        int unitLength = decodingState.decodeLength/2;
        ...
        for (int i=0;i<unitLength;i++){
            //a2,a3,f2(b),f2(b)+a1+a2+a3 计算a1
            byte a1 = (byte)(realInputs[0].get(i)^realInputs[1].get(i)^encodefBytebuffer[1].get(i+unitLength)^decodingState.inputs[7].get(i+unitLength));
        ...
        decodingState.outputs[0].limit(2*unitLength);
    }
